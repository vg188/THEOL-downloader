...
在北京化工大学 THEOL 教学平台的**当前课件列表**中，扫描、搜索、勾选 PDF / PPT / PPTX，再交给 Chrome 下载原文件。非学校官方扩展，不转换课件，不上传数据。

> 已通过 69 项自动化测试、Chrome 加载、模拟课程扫描及学校平台的一份真实 PPT 下载验证。真实 PDF/PPTX 下载尚未逐项实测；PPT 落盘容器存在字节差异，文稿和图片流校验一致。详细范围见 [验证记录](docs/verification.md)。
...
3. 点击“加载已解压的扩展程序”，选择**直接包含 manifest.json 的文件夹**。
   - 在本项目中，直接选择 `E:codexFile-scridistextension`。
   - 使用 ZIP 时，选择解压后包含 manifest.json 的目录，而不是其父目录。
4. 扩展列表应出现“北化课件助手”，版本 1.0.0。点击工具栏拼图图标，将它固定到工具栏。
...
2. 打开一门课程，进入“课程资源”中的课件目录，让文件列表加载完成，并保持这个课程标签页为当前页。
3. 点击工具栏的“北化课件助手”，再点击“扫描课件”。扫描只读取文件信息，**不会自动勾选或下载**。
...
- 默认保存到 **Chrome 配置的下载目录 / 课程名 / 原始文件名**。不能识别课程名时使用“课件”文件夹。
...
只支持 `https://course.buct.edu.cn/` 的已观察 THEOL 页面结构，只扫描**当前目录已加载的列表**：不自动进入子目录、不自动翻页、不跨课程扫描。换目录或翻页后重新扫描即可。
...
修改源码后，重新构建，再到 Chrome 扩展页点击本扩展的“重新加载”，并刷新课程页面、重新进入课件目录，避免旧内容脚本残留。**更新前先等当前任务结束。**
...
[查看 360 像素筛选与键盘焦点示例](docs/images/popup-360.png) · [验证结果与未验收项](docs/verification.md)
