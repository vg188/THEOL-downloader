...
> 已通过 69 项自动化测试、4 项原生 Chrome 弹出面板布局回归测试、Chrome 加载、模拟课程扫描及学校平台的一份真实 PPT 下载验证。真实 PDF/PPTX 下载尚未逐项实测；PPT 落盘容器存在字节差异，文稿和图片流校验一致。详细范围见 [验证记录](docs/verification.md)。
...
4. 刷新学校课程标签页，重新进入课件目录，再打开扩展并执行一次“扫描课件”。
