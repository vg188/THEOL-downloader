...
> 已通过 197 项源码自动化测试、4 项原生 Chrome 弹出面板布局回归测试、129 项自包含书签测试和 15 项站点构建测试。课程资源的一份真实 PPT 下载已完成验证；真实 PDF/PPTX 下载尚未逐项实测；PPT 落盘容器存在字节差异，文稿和图片流校验一致。单元学习模式目前只有合成夹具证据和真实页面的**只读元数据探针**结果（当前单元与全部单元均已放行），**真实单文件下载验收尚未执行**。详细范围见 [验证记录](docs/verification.md)。
